...
# 📊 TOTAL REAL DO CSV
st.metric("Total de registros de backups no CSV", len(df))
...
with st.sidebar:
   with st.sidebar:
    ...
    # 📅 Filtro por data
    data_inicio = st.date_input("📅 Data início", df["Dia"].min())
    data_fim = st.date_input("📅 Data fim", df["Dia"].max())
...
# ABAS
# =========================
aba1, aba2 = st.tabs(["📊 Gráficos", "📋 Dados"])
...
with aba1:
    ...
    col1.metric("❌ Falhas", len(falhas))
    col2.metric("💽 Alto Consumo", len(alto))
    col3.metric("⚠️ Risco Elevado", len(risco_alto))

    if len(falhas) > 0:
        st.error(f"🚨 {len(falhas)} backups com falha detectados!")
    ...
    col2.metric("Sucesso", total - falhas_total, delta="OK")
    col3.metric("Falhas", falhas_total, delta="- Problema", delta_color="inverse")
    ...
    crescimento = df.groupby("Dia")["GB"].sum().reset_index()
    crescimento["Media"] = crescimento["GB"].rolling(3).mean()

    fig4 = px.line(crescimento, x="Dia", y=["GB", "Media"], markers=True)
...
with aba2:
    ...
    st.subheader("📋 FILA DE BACKUPS")

    colunas_ordem = [
    "Tipo","Status_Icone","Status_Descricao","Backup","Categoria","Turno",
    "Modo","Inicio","Fim","Duracao","Tempo",
    ...
    .rename(columns={"Status_Icone": "Status","Status_Descricao": "Estado" })
    .style
